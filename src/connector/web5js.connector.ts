...
@Service({
  factory: (data: { id: string }) => {
    return new Web5jsConnector(data.id)
  },
  transient: true,
})
export class Web5jsConnector {
  ...
  private getBaseUrl() {
    ...
    if (process.env.WEB5JS_URL) {
      return process.env.WEB5JS_URL
    }
    return config.version === ApiVersion.V3 ? Constant.WEB5JS_API_URL.V3 : Constant.WEB5JS_API_URL.V4
  }
