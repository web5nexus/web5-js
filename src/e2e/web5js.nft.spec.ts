import { Ethereum, Network, Polygon, Web5jsSDK, Tezos } from '../service'
import { Utils } from '../util'

describe.skip('Web5JS NFT', () => {
  let client: Ethereum
  beforeAll(async () => {
    client = await Web5jsSDK.init<Ethereum>({
  ...
  describe('NFT balances', () => {
    it('should get NFT balances', async () => {
      ...
      expect(balance[1]).toStrictEqual({
        ...
        metadata: {},
        metadataURI: 'https://web5.nexus/1.json',
...
describe('Web5JS NFT - Tezos', () => {
  let client: Tezos

  beforeAll(async () => {
    client = await Web5jsSDK.init<Tezos>({
