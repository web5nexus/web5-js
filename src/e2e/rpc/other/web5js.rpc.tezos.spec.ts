import { Network, Web5jsSDK, Tezos } from '../../../service'
import { e2eUtil } from '../../e2e.util'

const getTezosRpc = async (testnet?: boolean) =>
  await Web5jsSDK.init<Tezos>(e2eUtil.initConfig(testnet ? Network.TEZOS_TESTNET : Network.TEZOS))

describe.each([false, true])(`Tezos`, (testnet: boolean) => {
  describe(`${testnet ? 'Testnet' : 'Mainnet'}`, () => {
    it('getBlock', async () => {
      const web5js = await getTezosRpc(testnet)
      const result = await web5js.rpc.getBlock({ block: 'head', chainId: 'main' })
      await web5js.destroy()
      expect(result).toBeDefined()
    })

    it('getBlocksHead', async () => {
      const web5js = await getTezosRpc(testnet)
      const result = await web5js.rpc.getBlocksHead({ chainId: 'main' })
      await web5js.destroy()
      expect(result).toBeDefined()
    })

    it('getCheckpoint', async () => {
      const web5js = await getTezosRpc(testnet)
      const result = await web5js.rpc.getCheckpoint({ chainId: 'main' })
      await web5js.destroy()
      expect(result).toBeDefined()
    })

    if (!testnet) {
      it('getContract', async () => {
        const web5js = await getTezosRpc(testnet)
        const result = await web5js.rpc.getContract({
          chainId: 'main',
          contractId: 'KT1Hkg5qeNhfwpKW4fXvq7HGZB9z2EnmCCA9',
          block: '3000000',
          normalizeTypes: true,
        })
        await web5js.destroy()
        expect(result).toBeDefined()
      })

      it('getContractBalanceAndFrozenBonds', async () => {
        const web5js = await getTezosRpc(testnet)
        const result = await web5js.rpc.getContractBalanceAndFrozenBonds({
          chainId: 'main',
          contractId: 'KT1Hkg5qeNhfwpKW4fXvq7HGZB9z2EnmCCA9',
          block: '3000000',
        })
        await web5js.destroy()
        expect(result).toBeDefined()
      })

      it('getContractsEntrypoints', async () => {
        const web5js = await getTezosRpc(testnet)
        const result = await web5js.rpc.getContractEntrypoints({
          chainId: 'main',
          contractId: 'KT1Hkg5qeNhfwpKW4fXvq7HGZB9z2EnmCCA9',
          block: '3000000',
        })
        await web5js.destroy()
        expect(result).toBeDefined()
      })
    }

    if (testnet) {
      // working with old data, operation bytes are not valid anymore
      it.skip('Inject Operation', async () => {
        const web5js = await getTezosRpc(testnet)
        const result = await web5js.rpc.injectOperation({
          operationBytes:
            '',
        })
        await web5js.destroy()
        expect(result).toBeDefined()
      })

      // TODO: Fix this test once will have relevant data https://tezos.stackexchange.com/questions/6309/how-to-properly-call-inject-block-or-protocol-api
      it.skip('Inject Block', async () => {
        const web5js = await getTezosRpc(testnet)
        const result = await web5js.rpc.injectBlock({
          data: '000',
          operations: [
            [
              {
                branch: 'BMPvGANiByxUeTkMJM1LtkEUNapeMcTjxuEAr9crBZdaYwbQX6a',
                data: '',
              },
            ],
          ],
        })
        await web5js.destroy()
        expect(result).toBeDefined()
      })

      // TODO: Fix this test once will have relevant data https://tezos.stackexchange.com/questions/6309/how-to-properly-call-inject-block-or-protocol-api
      it.skip('Inject Protocol', async () => {
        const web5js = await getTezosRpc(testnet)
        const result = await web5js.rpc.injectBlock({
          data: '000',
          operations: [
            [
              {
                branch: 'BMPvGANiByxUeTkMJM1LtkEUNapeMcTjxuEAr9crBZdaYwbQX6a',
                data: '',
              },
            ],
          ],
        })
        await web5js.destroy()
        expect(result).toBeDefined()
      })
    }
  })
})
