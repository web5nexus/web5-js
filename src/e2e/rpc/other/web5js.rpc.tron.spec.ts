...
import { Network, Web5jsSDK, Tron } from '../../../service'
import { e2eUtil } from '../../e2e.util'

const getTronRpc = async (testnet?: boolean) =>
  await Web5jsSDK.init<Tron>(e2eUtil.initConfig(testnet ? Network.TRON_SHASTA : Network.TRON, testnet ? process.env.V3_API_KEY_TESTNET : process.env.V4_API_KEY_MAINNET))

describe('RPCs', () => {
  describe('TRON', () => {
    describe('testnet', () => {
      it('getNowBlock', async () => {
        const web5js = await getTronRpc(true)
        const result = await web5js.rpc.getNowBlock()
      ...
      it('getChainParameters', async () => {
        const web5js = await getTronRpc(true)
        const result = await web5js.rpc.getChainParameters()
      ...
      it('getBlockByNum', async () => {
        const web5js = await getTronRpc(true)
        const result = await web5js.rpc.getBlock('1000000')
        expect(result.block_header.raw_data.number).toBeGreaterThan(0)
        await web5js.destroy()
      })
      ...
      it('getBlockById', async () => {
        const web5js = await getTronRpc(true)
        const result = await web5js.rpc.getBlock(
      ...
      it('getTransactionById', async () => {
        const web5js = await getTronRpc(true)
        const result = await web5js.rpc.getTransactionById(
    ...
    describe('mainnet', () => {
      it('getNowBlock', async () => {
        const web5js = await getTronRpc(false)
        const result = await web5js.rpc.getNowBlock()
        expect(result.block_header.raw_data.number).toBeGreaterThan(0)
        await web5js.destroy()
      })
      ...
      it('getBlockByNum', async () => {
        const web5js = await getTronRpc(false)
        const result = await web5js.rpc.getBlock('51173114')
        expect(result.block_header.raw_data.number).toBeGreaterThan(0)
        await web5js.destroy()
      })
      ...
      it('getBlockByLimitNext', async () => {
        const web5js = await getTronRpc(false)
        const result = await web5js.rpc.getBlockByLimitNext(1, 5)
      ...
      it('getAccountBalance', async () => {
        ...
        const res = await web5js.rpc.getAccountBalance(accountIdentifier, blockIdentifier, { visible: true })
