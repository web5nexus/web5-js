...
export const Utils = {
  getRpc: <T>(id: string, config: Web5jsConfig): T => {
  ...
  getV3RpcUrl: (config: Web5jsConfig, path?: string): string => {
    const { apiKey, rpc, network } = config
    if (apiKey) {
      const url =
        rpc?.nodes?.[0].url ||
        `${Constant.WEB5JS_API_URL.V3}blockchain/node/${network}/${apiKey.v3 ? apiKey.v3 : apiKey.v4}`
      return url.concat(path || '')
    }
    return rpc?.nodes?.[0].url || `${Constant.WEB5JS_API_URL.V3}blockchain/node/${network}`.concat(path || '')
  },
  addQueryParams: (
    basePath: string,
    strategy: (key: string) => string,
