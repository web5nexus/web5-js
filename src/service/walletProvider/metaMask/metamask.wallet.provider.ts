...
export class MetaMask extends Web5jsSdkWalletProvider<string, TxPayload> {
  supportedNetworks: Network[] = EVM_BASED_NETWORKS
  private readonly config: Web5jsConfig
  private readonly rpc: EvmRpc
  private readonly connector: Web5jsConnector
  private readonly logger: Logger

  constructor(web5jsSdkContainer: IWeb5jsSdkContainer) {
    super(web5jsSdkContainer)
    this.config = this.web5jsSdkContainer.getConfig()
    this.rpc = this.web5jsSdkContainer.get(EvmRpc)
    this.connector = this.web5jsSdkContainer.get(Web5jsConnector)
    this.logger = this.web5jsSdkContainer.getLogger()
  }
  ...
  async getWallet(): Promise<string> {
    ...
    if (EnvUtils.isDevelopment()) {
      this.logger.info(
        'You can get FREE testnet tokens to test your contracts on any number of chains: https://docs.web5.nexus',
      )
    }
